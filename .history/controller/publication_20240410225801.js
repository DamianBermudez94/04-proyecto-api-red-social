...
const upLoad = async (req, res) => {
  try {
    ...
    let publicationUpdate = await Publication.findOneAndUpdate(
      { user: req.user.id,_id:publication},
    ...
    return res.status(200).json({
      status: "success",
      mensaje: "La imagen se ha actualizado correctamente",
      user: req.user,
    ...
    return res.status(404).json({
      status: "Error",
      message:
        "Error: Tiene que subir una imagen para poder actualizar",
    });
  }
};
...
const feed = async (req, res) => {
  try {
    ...
    const myFollows = await FollowServices.followUserIds(userId);
