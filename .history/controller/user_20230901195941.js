  ...
  User.find({
    ...
    if (error) {
      return res
        ...
        .json({
          error: "Error",
          message: "Error en la consulta de los usuarios",
        });
    }
    ...
    const pwd = bcrypt.hash(parametros.password,10);
    ...
    nuevoUsuario.save((error, userSave)=>{
      if(error) return res.status(500).send({error:"error", message:"Error al cargar el usuario"})
