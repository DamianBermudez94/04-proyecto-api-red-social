...
const Login = async (req, res) => {
  ...
  if (!params.email || !params.password) {
    return res
      .status(500)
      .send({ error: "error", message: "el usuario no existe" });
  }
  ...
  let pwd = bcrypt.compareSync(params.id, user.password)
if (pwd) {
  return res
  .status(500)
  .send({ error: "error", message: "No te has identificado correctamente" });
}
