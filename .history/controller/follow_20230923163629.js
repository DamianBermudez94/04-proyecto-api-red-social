...
const unfollow = async (req, res) => {
  ...
  let followDelete = await Follow.find({
    user: userId,
    followId: followId,
  })

  let deleteFollow = followDelete.findOneAndDelete( filter, options )
    if (!deleteFollow) {
      return res.status(500).json({
        status: "error",
        mensaje: "Error al dejar de seguir al usuario, verificar la solicitud enviada",
      });
    }
    return res.status(200).json({
      status: "success",
