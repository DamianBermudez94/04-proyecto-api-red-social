...
const detail = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: "success",
      mensaje: "Información sobre la publicicón",
...
const listPublication = async (req, res) => {
  try {
    // Extraemos el ID del usuario (si no se proporciona, usamos el ID del usuario logueado)
    const userId = req.params.id || req.user.id;
    console.log("soy el id del usuario", userId);
    // Establecemos la página (si no se proporciona, usamos la página 1)
    const page = parseInt(req.params.page) || 1;

    // Limitamos el número de usuarios por página
    const itemsPerPage = 5;

    // Consulta las publicaciones del usuario utilizando mongoose paginate
    const listPublications = await Publication.paginate(
      {},
      { page, limit: itemsPerPage }
    );

    // Verificamos si hay publicaciones
    if (listPublications.docs.length === 0) {
      return res.status(404).json({
        status: "Error",
    ...
    // Devolvemos la respuesta con los datos paginados
    return res.status(200).json({
      status: "success",
      ...
      listPublications,
      itemsPerPage,
      total: listPublications.totalDocs,
      page,
      pages: listPublications.totalPages,
    });
  } catch (error) {
    // Manejamos errores internos
    return res.status(500).json({
      status: "Error",
      message: "Hubo un problema al obtener las publicaciones del usuario",
    });
  }
};
...
const upLoad = async (req, res) => {
  try {
    ...
    let publicationId = req.params.id;
    console.log("soy el ID de la publicacion", publicationId);
    ...
    if (
      ...
      fs.unlink(req.file.path, (error) => {
        return res.status(404).json({
          status: "error",
          mensaje:
            "La imagen que quiso subir no es valida, comprobar que sea con los siguientes formatos:PNG,JPG,JPEG,GIF",
        });
      });
    }
    ...
    // Metodo findOneAndUpdate: sirve para editar/actulizar un elemento de la base de datos, le pasamos como parametros: el id del usuario y el de la publicacion,y d la imagen a actualizar
    ...
    console.log("soy la imagen que quiero actualizar", publicationUpdate._id.publicationId);
