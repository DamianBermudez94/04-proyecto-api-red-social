...
const Login = async (req, res) => {
  ...
  const userExiste = await User.findOne({ email: parametros.email })
    .select({ password: 0 }).exec()
 
  console.log("El usuario existe", userExiste);
  try {
    const results = await userExiste;
    ...
    return res.status(400).send({
      status: "Error",
      message:
        "Error, el usuario no existe o se cargaron mal los datos. Verificar los datos enviados",
    });
  }
};
