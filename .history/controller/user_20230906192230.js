...
const listadoUser = async (req, res) => {
  ...
  const userList = User.find().sort("_id").paginate(page, itemsPerPage,total);
  userList.then((users,total)=>{
    console.log("soy la lista de usuarios:",users)
    console.log("soy el total de usuarios:",total)
    if (!users) {
