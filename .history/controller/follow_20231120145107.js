...
const listadoFollow = async (req, res) => {
  try {
    ...
    const followlist = await Follow.find({ user: userId })
      ...
      .exec();
    console.log("soy el follow list",followlist);
    let followUserIds = await followServicie.followUserIds(userId);
    console.log("soy el followUser",followUserIds);
    return res.status(200).json({
      status: "success",
      ...
      user_following: followlist.following,
...
const followers = async (req, res) => {
  try {
    ...
    const followlist = await Follow.find({ followed: userId }).populate(
      "user",
      "-password -role -__v -email"
    );

    console.log("hollaaaa", followlist);
    if (!followlist) {
      return res.status(404).json({
        status: "Error",
        message: "No hay usuarios disponibles",
