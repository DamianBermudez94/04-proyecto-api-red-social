  ...
  try {
    ...
    if (!userProfile) {
      return res.status(400).json({
        status: "error",
        mensaje:
          "No se ha podido encontrado al usuario, por favor verificar los datos enviados",
      });
    }

    // info de seguimiento
    // como parametros le pasamos el id del usuario logueado y el id del perfil del usuario
    const followInfo = await followServicie.followThisUser(req.user._id, id);
    return res.status(200).json({
      status: "success",
      user: userProfile,
...
const listadoUser = async (req, res) => {
  try {
    ...
    const userlist = await User.paginate({}, { page, limit: itemsPerPage });
    
    console.log(userlist.docs); // Los documentos de la página actual
    console.log(userlist.totalDocs); // Total de documentos
    console.log(userlist.totalPages); // Total de páginas
    
  
   
  
    console.log("soy la userlist", userlist);
    if (!userlist) {
    ...
    console.log("hola", followUserIds);
    return res.status(200).json({
      status: "success",
