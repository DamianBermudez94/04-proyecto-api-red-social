...
const listPublication = async (req, res) => {
  ...
  console.log(userId);
    ...
    return res.status(200).json({
      ...
      page,
      pages: Math.ceil(listPublications.totalPages / itemsPerPage),
    });
  } catch (error) {
    return res.status(500).json({ // Cambio de estado a 500 para errores internos
      status: "Error",
      message:
        "Hubo un problema al obtener las publicaciones del usuario",
    });
};
