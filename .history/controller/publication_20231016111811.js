...
const listPublication = async (req,res)=>{
  try {
    ...
    const listPublications = await Publication.find({user:userId}).sort("-created_at").populate().paginate(page, itemsPerPage).exec()
...
const upLoad = async (req, res) => {
  ...
  console.log("Hola mundo", publicationUpdate);
...
try {
  ...
  //* Sacar un array de id de usuarios que yo sigo como usuario logueado
  const myFollws = await FollowServices.followUserIds(req.user.id);

  // Consulta con mongoose paginate

  // Como parametro le pasamos UserId ( usuario logueado )
  const listPublications = await Publication.find({user:myFollws.following}).populate("user","-password -role -__v -email").sort("-created_at").paginate(page, itemsPerPage).exec()
